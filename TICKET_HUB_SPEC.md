...
- validate 逻辑:
  1. getTransition(currentStatus, action) → 没有则 BadRequestException，**错误信息包含可用动作列表**: `Cannot perform '${action}' on ticket with status '${currentStatus}'. Available: ${getAvailableActions(currentStatus).join(', ')}`
  2. 检查 user.permissions 包含 transition.permission → 否则 ForbiddenException
  3. assign 操作必须提供 assigneeId → 否则 BadRequestException
  4. 非 assign 操作时，`assigneeId` 保留原值 `ticket.assigneeId`（不置空）
- **事务边界**（关键）: SLA 取消、缓存失效、事件发射**必须在事务 commit 之后**执行，不能放在事务内部。原因：如果事务回滚，但缓存已删除/事件已发出，会导致脏状态。
- SLA 取消逻辑（事务外）:
  - 状态变为 ASSIGNED / IN_PROGRESS → `slaService.cancelResponseCheck(ticketId)`
  - 状态变为 RESOLVED / CLOSED → `slaService.cancelAllChecks(ticketId)`
- 缓存失效（事务外）: `await this.invalidateCache(ticketId)`
- 事件发射（事务外）: `this.eventEmitter.emit('ticket.transitioned', new TicketTransitionedEvent(...))`
...
### 集成点（TicketModule 必须导入 SlaModule）

TicketModule 的 `imports` 数组中必须包含 `SlaModule`，TicketService 构造函数必须注入 `SlaService`。

具体调用关系：
- `TicketService.create()` → 事务提交后调用 `slaService.scheduleChecks(ticket.id, ticket.priority)`
- `TicketService.transition()` → 事务提交后:
  - 新状态为 ASSIGNED/IN_PROGRESS → `slaService.cancelResponseCheck(ticketId)`
  - 新状态为 RESOLVED/CLOSED → `slaService.cancelAllChecks(ticketId)`
- `TicketService.grabTicket()` → 成功后调用 `slaService.cancelResponseCheck(ticketId)`
...
| Module | exports | Class-level Guards | 说明 |
|--------|---------|-------------------|------|
| AuthModule | AuthService, JwtAuthGuard | - | 被多个模块导入使用 |
| RbacModule | RbacService | JwtAuthGuard | AuthService 调用 getUserPermissions() |
| TicketModule | TicketService | JwtAuthGuard, PermissionGuard | 每个路由加 @RequirePermissions, **imports SlaModule** |
| DepartmentModule | DepartmentService | JwtAuthGuard, PermissionGuard | |
| NotifyModule | NotifyGateway | - | 导出 Gateway 供其他模块注入（实际由 listener 内部调用） |
| SlaModule | SlaService | - | TicketModule 导入并在 TicketService 中注入使用 |
| AuditModule | - | - | @Global, APP_INTERCEPTOR 自动生效 |

### NotifyModule 的 JWT 复用

NotifyModule 通过导入 AuthModule 复用 JWT 配置，**不要**自行注册 JwtModule。

---

## 关键实现决策 (L3 层)

以下是 AI 容易"合理推测但推测错"的二选一决策，必须严格遵守：

### 1. RBAC assign 语义：替换（非追加）

```typescript
// assignPermissionsToRole: 先删旧关联，再插新关联（replace 语义）
await tx.delete(rolePermissions).where(eq(rolePermissions.roleId, roleId));
if (permissionIds.length > 0) {
  await tx.insert(rolePermissions).values(permissionIds.map(...));
}

// assignRolesToUser: 同理，先删后插
await tx.delete(userRoles).where(eq(userRoles.userId, userId));
```

`assignPermissionsToRole(roleId, permissionIds[])` 接收两个参数，不是 DTO 对象。  
`assignRolesToUser(userId, roleIds[])` 同理。  
Controller 层负责从 DTO 解构后传入。

### 2. RbacService 必须包含 getUserPermissions()

```typescript
// RbacService.getUserPermissions(userId): string[]
// JOIN userRoles → rolePermissions → permissions
// 返回去重的 "resource:action" 格式数组
// AuthService.generateTokens() 调用此方法获取权限列表嵌入 JWT
```

### 3. DataScopeInterceptor 的 ticket:view_all 跳过

```typescript
// 如果 user.permissions 包含 'ticket:view_all' → req.dataScope = undefined（不限制）
// 否则 → 查 userDepartments + closure 获取部门 IDs 赋值给 req.dataScope
```

### 4. TicketController 的权限装饰器

```
POST /tickets         → @RequirePermissions('ticket:create')
GET  /tickets         → (无 RequirePermissions，依赖 DataScopeInterceptor 做范围控制)
GET  /tickets/:id     → @RequirePermissions('ticket:view')
POST /tickets/:id/transition → (权限由状态机 transition.permission 动态校验)
GET  /tickets/:id/logs → @RequirePermissions('ticket:view')
```

### 5. HTTP 动词

- Department update: `PUT /:id`（全量更新语义）
- Department move: `PUT /:id/move`
- 所有 `:id` 参数加 `ParseUUIDPipe` 校验

### 6. 全局异常过滤器的 message 解析

```typescript
// HttpException 时：调用 exception.getResponse()
// 如果 response 是对象且有 message 字段 → 使用该 message
// 否则 → 使用 exception.message
// 非 HttpException 时：用 Logger 记录 stack trace，返回 500 + 'Internal server error'
```

### 7. 排序方向

- `ticket.findAll()`: orderBy `createdAt` **desc**（最新在前）
- `audit.findAll()`: orderBy `createdAt` **desc**（最新在前）
- `department.findAll()`: orderBy `createdAt` **asc**（创建顺序）
- `ticket.getLogs()`: orderBy `createdAt` **asc**（时间线顺序）

### 8. Department service 细节

- 父节点不存在时抛 `NotFoundException`（404，不是 400）
- `remove()` 返回 `{ success: true }`
- 闭包表维护：先收集所有行到 JS 数组，再一次性 `insert().values(rows)`（非 raw SQL INSERT...SELECT）
- `findAll` 的 count 使用 `` sql<number>`count(*)::int` ``

### 9. DTO 文件组织

每个 DTO 独立文件（如 `create-role.dto.ts`、`assign-permissions.dto.ts`），**不要** barrel 合并到 `index.ts`。

### 10. Logger 使用

- 所有 Module 级日志使用 NestJS `Logger`（`new Logger(ClassName.name)`），**不用** `console.log/warn`
- 包括：GlobalExceptionFilter、RequestLoggerMiddleware、SlaProcessor、NotifyGateway

### 11. seed.ts 实现

- 导入方式：`import { createDrizzle } from './index'` + 按名导入 schema 表
- Permission 记录包含 `description` 字段
- Role 插入：批量一次性 insert，role-permission 关联先收集到数组再批量 insert
- **幂等性**：所有 insert 使用 `.onConflictDoNothing()`，确保 seed 可重复执行
- 结束时 `process.exit(0)`

### 12. 状态机类型

```typescript
// 内层 Record 使用 Record<TicketAction, Transition>（严格类型）+ as any 转型
const TRANSITIONS: Record<string, Record<TicketAction, Transition>> = { ... } as any;
```

### 13. package.json 要点

- jest 配置中包含 `"setupFiles": ["./test/setup.ts"]`
- test script 使用 `jest --runInBand`
- 包含 `"db:seed": "ts-node src/db/seed.ts"` script

---

## Phase 9: Performance + Cache + Idempotency

### 9.1 Redis Module (src/redis/redis.module.ts)

独立模块，`@Global()`，使用 `forRootAsync()` 动态模块模式：
...
AppModule 中注册：`RedisModule.forRootAsync()`（与 BullModule.forRootAsync 风格统一）。
...
CREATE INDEX idx_tickets_dept_created ON tickets (department_id, created_at DESC);
```

### 9.3 性能优化 — TicketService.findAll 重写

**近似计数**: 无过滤条件时用 `pg_class.reltuples` 替代 `count(*)`：

```typescript
if (!whereClause) {
  const result = await this.db.execute(
    sql`SELECT reltuples::int AS estimate FROM pg_class WHERE relname = 'tickets'`,
  );
  total = (result.rows[0] as any).estimate;
}
```

**延迟 JOIN**: 先在索引上取 ID，再回表取完整行：
...
).then((r) => r.rows as (typeof tickets.$inferSelect)[])
```

有 whereClause (dataScope) 时，子查询内加 WHERE 条件。

**排序方向**: `ORDER BY created_at DESC`（最新在前）。

### 9.4 Redis 缓存 — TicketService.findOne

**Cache-Aside 模式**:

```typescript
private static CACHE_TTL = 300; // 5 minutes
private static CACHE_PREFIX = 'ticket:';

async findOne(id: string) {
  const cacheKey = `${TicketService.CACHE_PREFIX}${id}`;
  ...
  await this.redis.setex(cacheKey, TicketService.CACHE_TTL, JSON.stringify(ticket));
  return ticket;
}
```

**延迟双删** — transition/grab 执行后调用：

```typescript
private async invalidateCache(ticketId: string) {
  const cacheKey = `${TicketService.CACHE_PREFIX}${ticketId}`;
  await this.redis.del(cacheKey);
  setTimeout(() => this.redis.del(cacheKey), 500);
}
```

TicketService 构造函数注入 `@Inject(REDIS_CLIENT) private redis: Redis`。

### 9.5 接口幂等性 (src/common/interceptors/idempotency.interceptor.ts)

**NestJS Interceptor**，注入 `REDIS_CLIENT`。

逻辑：
1. 读取请求头 `idempotency-key`，无则放行
2. 查 `idem:result:{key}`，有则直接返回缓存结果
3. `SET idem:lock:{key} 1 EX 600 NX`，失败则 throw `ConflictException('Request is being processed')`
4. 放行执行业务，响应后通过 `tap()` 将结果 `SETEX idem:result:{key} 600` 存入 Redis
...
**挂载位置**：`@UseInterceptors(IdempotencyInterceptor)` 应用于：
- `POST /tickets`（创建工单）
- `POST /tickets/:id/transition`（状态流转）

### 9.6 Bulk Seed (src/db/seed-bulk.ts)

插入 100 万 tickets 用于压测：
- 200 个 users, 50 个 departments
- 随机 status/priority/creatorId/assigneeId/departmentId
- 每批 5000 条，循环 insert
- package.json 添加 `"db:seed-bulk": "ts-node src/db/seed-bulk.ts"`

### 9.7 Benchmark (benchmark/perf-test.js)

k6 脚本，三个场景顺序执行：
- `list_tickets`: 20 VU, 30s — 分页列表（随机页码）
- `get_ticket`: 10 VU, 30s — 单条查询（缓存命中）
- `grab_ticket`: 50 VU, 15s — 并发抢单（同一批 ticket）

Thresholds:
- `list_tickets` P95 < 200ms
- `get_ticket` P95 < 50ms
...
15. 测试代码和业务代码同步编写
16. **RedisModule 用 forRootAsync() 模式**，导出 REDIS_CLIENT Symbol，独立目录 `src/redis/`
17. **findAll 用延迟 JOIN + 近似计数**，不要用 ORM 的 `.select().from().orderBy().limit().offset()` 直接查（PG 对宽表不走索引）
18. **findOne 加 Cache-Aside**，写操作后调用 invalidateCache（延迟双删）
19. **IdempotencyInterceptor 只挂在 POST 创建和 POST transition 上**，不是全局
20. **事务边界原则**: 缓存失效（invalidateCache）、事件发射（eventEmitter.emit）、SLA 调度/取消必须在数据库事务 **commit 之后**执行。事务内只做数据库操作（SELECT FOR UPDATE → UPDATE → INSERT log），副作用一律放事务外。
21. **seed.ts 必须幂等**: 所有 insert 使用 `.onConflictDoNothing()`，确保可重复执行不报错
22. **错误信息包含上下文**: BadRequestException 应包含当前状态和可用操作列表，帮助调用方定位问题
